...
- Prefer `codex mcp add surrealdb -- surreal mcp stdio` when the user is working against a local SurrealDB CLI installation.
...
- Do not guess between stdio, local HTTP, Docker, or Cloud. The user-provided transport is the source of truth.
...
- For HTTP bearer auth, prefer `codex mcp add surrealdb --url <endpoint> --bearer-token-env-var <ENV_VAR>` only when the env var contains a ready-to-use access token or JWT.
- A `surreal-bearer-...` bearer grant key is not the same thing as the final HTTP auth token for `/mcp`; it typically must be exchanged through SurrealDB signin first.
...
- Connect to my local SurrealDB instance with `surreal mcp stdio`.
