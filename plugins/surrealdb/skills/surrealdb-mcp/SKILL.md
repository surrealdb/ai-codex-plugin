---
name: surrealdb-mcp
description: Use when the user asks Codex to inspect, query, administer, or troubleshoot SurrealDB through a user-provided SurrealDB MCP endpoint URL.
---

# SurrealDB MCP

Use this skill when the user asks Codex to inspect, query, administer, or troubleshoot SurrealDB through MCP.

## MCP Server

- Ask the user for their SurrealDB MCP endpoint URL when it is not already configured.
- Configure the server directly in Codex with `codex mcp add surrealdb --url <endpoint>`.
- Do not guess between local CLI, Docker, or Cloud. The user-provided URL is the source of truth.

## Configuration Notes

- Use a direct MCP URL such as `https://<cloud-instance>/mcp` or `http://127.0.0.1:8000/mcp`.
